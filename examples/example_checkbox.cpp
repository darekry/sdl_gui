...
int main(int, char**) {
    try {
        ...
        panel->setStyle(ElementState::Normal, {.backgroundColor = {{50, 50, 50, 255}}, .textColor = std::nullopt, .texture = std::nullopt, .borderColor = std::nullopt, .borderWidth = std::nullopt, .borderRadius = std::nullopt, .fontSize = std::nullopt, .fontName = std::nullopt});

        auto checkbox = std::make_unique<Checkbox>(guiManager, 10, 10, 20, 20);
        checkbox->setOnChange([](Checkbox*, bool isChecked) {
            LOG_INFO("Checkbox", "Checkbox state changed: {}", isChecked ? "Checked" : "Unchecked");
        });
